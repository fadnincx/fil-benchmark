...
func (_ K3s) GetLotusHosts() []fil_benchmark_exec.Node {
	checkIsRoot()
	err, out, _ := LocalCmd("k3s kubectl get pods -o wide | grep lotus-node | awk '{print $6}' | wc -l")
	if err != nil {
	...
	for i := 0; i < amount; i++ {
		err, ip, _ := LocalCmd(fmt.Sprintf("k3s kubectl get pods -o wide | grep lotus-node-%d | awk '{print $6}'", i))
		if err != nil {
			log.Printf("error: %v\n", err)
		}
		resultNode[i].Ip = strings.TrimSuffix(ip, "\n")
		err, hostname, _ := LocalCmd(fmt.Sprintf("k3s kubectl get pods -o wide | grep lotus-node-%d | awk '{print $1}'", i))
		if err != nil {
			log.Printf("error: %v\n", err)
		}
...
func (_ K3s) GetRedisHost() string {
	...
	err, out, _ := LocalCmd("k3s kubectl get pods -o wide | grep lotus-redis | awk '{print $6}' | wc -l")
	if err != nil {
		log.Printf("error getting pod amount: %v\n", err)
	}
	amount, _ := strconv.Atoi(strings.TrimSuffix(out, "\n"))
	...
	if amount > 0 {
		err, ip, _ := LocalCmd(fmt.Sprintf("k3s kubectl get pods -o wide | grep lotus-redis-0 | awk '{print $6}'"))
		if err != nil {
			log.Printf("error: %v\n", err)
		}
